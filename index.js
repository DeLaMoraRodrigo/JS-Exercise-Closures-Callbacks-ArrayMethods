...
function getFullNames(runners) {
  let fullNames = [];
  runners.forEach((val) => {
    fullNames.push(val.last_name + ", " + val.first_name);
  });
  return fullNames;
}
...
function firstNamesAllCaps(runners) {
  let firstNamesCaps = runners.map((val) => {return val.first_name.toUpperCase();});
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let filteredTShirts = runners.filter((val) => {return val.shirt_size === tShirtSize});
...
function tallyUpDonations(runners) {
  let sum = runners.reduce((acc, val) => {return acc + val.donation;}, 0);
  return sum;
}
